...
def scalar_transform(scalar, X):
    shape = X.shape
    X_ravel = np.array([i.ravel() for i in X])
    tmp = scalar.transform(X_ravel)
    out = tmp.reshape(shape)
    return out
...
if __name__ == '__main__':
    ...
    train_data_tensor, valid_data_tensor, test_data_tensor = trans(train_data_tensor, valid_data_tensor,
                                                                   test_data_tensor)
    savepath = r"models/"
    RNN = RNNModule('GRU', 11, 26, 26, 3, add_linear=True)
    rnn_pre = train(RNN, train_data_tensor, train_labels_tensor, epochs=40, batch_size=4, clip=None,
