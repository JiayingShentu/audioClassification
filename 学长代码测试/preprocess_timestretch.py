...
if __name__ == '__main__':
    ...
    for i in os.listdir(files):
        for file in os.listdir(files+i+"\\"):
            ...
            signal_time_stretch=y_ts = librosa.effects.time_stretch(signal, rate=1.1)
            ...
            signal_time_stretch_mfcc =get_mfcc(signal_time_stretch,sr,0.4,0.4)
            label=i
            label_num=np.ones(signal_mfcc.shape[0]) * label_dict[label]
            label_num_plus=np.ones(signal_time_stretch_mfcc.shape[0]) * label_dict[label]
            #print(i,file,label,label_num)

            if num == 0:
                signal_mfccs = signal_mfcc
                label_nums = label_num
                signal_mfccs = np.concatenate([signal_mfccs, signal_time_stretch_mfcc], axis=0)
                label_nums = np.concatenate([label_nums, label_num_plus])
            else:
    ...
    np.save(r'D:\毕设相关\audioClassification\学长代码测试\data\timeStretch_train_data.npy', train_data)
    np.save(r'D:\毕设相关\audioClassification\学长代码测试\data\timeStretch_train_label.npy', train_label)
    np.save(r'D:\毕设相关\audioClassification\学长代码测试\data\timeStretch_valid_data.npy', valid_data)
    np.save(r'D:\毕设相关\audioClassification\学长代码测试\data\timeStretch_valid_label.npy', valid_label)
